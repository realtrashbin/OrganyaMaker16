...
* V1.2: 2026-xxx-xx
* - Added support for Org-16 files ~Bi_N
* TODO:
...
struct XMHeader
{
    ...
    moduleName = "Extended Module: ", 
        "", 
        0x1A, "Org2XM by Rrrola    ",
        0x104,
        0x114
} xmh;
...
int main(int argc, char** argv)
{
    ...
    if        (memcmp(header.magic, "Org-02", 6) || memcmp (header.magic, "Org-01",6)) {
        verorg = 2;
    }
    else if (memcmp(header.magic, "Org-16", 6)) {
        verorg = 4;
    }
    ...
    //errors
    Err6: MessageBox(hWnd,"Bank version smaller than org version!\n","Error(Org2XM)",MB_OK);                   return 6;
    Err5: MessageBox(hWnd,"Speed out of XM range!\n","Error(Org2XM)",MB_OK);                   return 5;
    Err3: MessageBox(hWnd,"Invalid org header!\n","Error(Org2XM)",MB_OK);                   return 3;
    Err2: MessageBox(hWnd, printf("Couldn't open file \"%s\"!\n", argv[1]), "Error(Org2XM)", MB_OK);                   return 2;
    Err1: MessageBox(hWnd, printf("Usage: \"org2xm infile.org ORGxxxyy.DAT\"\n"
    "    OR \"org2xm infile.org ORGxxxyy.DAT c\" (for compatible output)"), "Error(Org2XM)", MB_OK);;                   return 1;
}
